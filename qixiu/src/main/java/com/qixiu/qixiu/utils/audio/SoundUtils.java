...
public class SoundUtils {
    ...
    public static void playSoundByMedia(int rawId) {
        try {
            ...
            try {
                AssetFileDescriptor file = BaseApplication.getInstance().getResources().openRawResourceFd(
                        rawId);
    ...
    public static void playSound(int rawId) {
        ...
        soundPool.load(BaseApplication.getInstance(), rawId, 1);
...
//        soundPool.play(1, 1, 1, 0, 0, 1);
    ...
    public static int soundResourse;

    public static void playMusic(Context context, int resoure) {
        Intent intent = new Intent(context, PlayMusicService.class);
        soundResourse = resoure;
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        context.startService(intent);
    }

    public static int getSoundResourse() {
        return soundResourse;
    }
